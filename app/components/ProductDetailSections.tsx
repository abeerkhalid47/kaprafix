...
export default function ProductDetailSections() {
  ...
  useGSAP(() => {
    // 1. How-to steps — stagger slide from left
    ...
    // 4. Fabric pills — scatter in
    gsap.from('.pds-fabric-pill', {
      scrollTrigger: { trigger: fabricRef.current, start: 'top 80%', toggleActions: 'play none none reverse' },
      y: 20, opacity: 0, scale: 0.85, stagger: 0.06, duration: 0.45, ease: 'back.out(1.5)',
    });

    // 5. Why KapraFix — chained timeline
    const whyTl = gsap.timeline({
      scrollTrigger: { trigger: whyRef.current, start: 'top 60%', toggleActions: 'play none none reverse' },
    });
    whyTl
